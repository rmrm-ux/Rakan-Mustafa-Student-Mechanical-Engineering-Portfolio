...
R_cool_bottom = 0.6;   % Thermal resistance from each cell to the bottom chill plate (K/W)
...
% Every cell now has its own path to the coolant, so Cell 1 and Cell 10
% no longer connect to a shared end boundary the way they did in the
% end cooled version, both ends stay insulated laterally, and heat
% instead leaves straight down through R_cool_bottom at every cell.
for k = 1:(length(time)-1)
    ...
    for i = 1:N_cells
        ...
        Q_cool = (T(i, k) - T_cool) / R_cool_bottom;

        dTdt = (Q_gen - Q_left - Q_right - Q_cool) / (m_cell * Cp_cell);
...
xlabel('Cell Number (Cell 1: Insulated Base | Cell 10: Bottom Cooled)', 'FontSize', 11, 'Color', 'k');
...
title('Transient Temperature Response — Bottom-Cooled Architecture', 'FontSize', 14, 'Color', 'k', 'FontWeight', 'bold');
